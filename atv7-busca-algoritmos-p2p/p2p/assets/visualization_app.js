...
function parseConfigFileText(text, filename = "") {
  ...
  const isJson = filename.toLowerCase().endsWith(".json") || /^[\[{]/.test(content);
  const config = isJson ? JSON.parse(content) : parseSimpleYamlConfig(content);
  return normalizeNetwork(configToNetworkInput(config));
}
...
function normalizeNetwork({ resourcesByNode, edges, configuredCaches }) {
  const nodes = Array.from(resourcesByNode.keys()).sort(compareNodeIds);
...
async function loadConfigFile() {
  const file = controls.configFile.files && controls.configFile.files[0];
  if (!file) return;

  try {
    const text = await file.text();
    const network = parseConfigFileText(text, file.name);
    fillEditorFromNetwork(network);
    activeNetwork = network;
    normalizeCurrentQueryForNetwork(activeNetwork);
    executeFromInterface(`Arquivo ${file.name} carregado.`);
  } catch (error) {
    stop();
    setEditorStatus(error.message, "error");
  }
...
controls.configFile.addEventListener("change", loadConfigFile);

for (const input of [controls.meshResources, controls.meshEdges, controls.meshCaches]) {
